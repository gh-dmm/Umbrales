...
class AplicacionMuseo {
    constructor(contenedorId) {
        ...
        // Rutas directas a tus archivos en la subcarpeta data/
        this.RUTAS_EXCEL = {
    ...
    /**
     * CONSTRUCTOR DE INTERFAZ: Inyecta dinámicamente todo el HTML necesario al DOM
     */
    inicializarEstructuraBase() {
        this.root.innerHTML = `
            <div class="container py-5">
                <h1 class="text-center mb-4 text-warning fw-bold">Sistema de Acervos Culturales (Engine POO)</h1>
                <p class="text-center text-muted mb-5">Interfaz generada al 100% dinámicamente desde JavaScript utilizando encapsulamiento de objetos.</p>

                <div class="row justify-content-center gap-2 mb-4">
                    <div class="col-auto">
                        <button class="btn btn-primary shadow-sm fw-bold" onclick="AppMuseo.solicitarCargaExcel(1, 'inah_museos', 'loteria')">
                            🏛️ Museo INAH (ID: 1 - Lotería)
                        </button>
                    </div>
                    <div class="col-auto">
                        <button class="btn btn-success shadow-sm fw-bold" onclick="AppMuseo.solicitarCargaExcel(2, 'monumentos', 'ficha')">
                            🏛️ Monumento (ID: 2 - Ficha)
                        </button>
                    </div>
                    <div class="col-auto">
                        <button class="btn btn-info text-dark shadow-sm fw-bold" onclick="AppMuseo.solicitarCargaExcel(3, 'objetos_externos', 'loteria')">
                            🏛️ Objeto Externo (ID: 3 - Lotería)
                        </button>
                    </div>
                    <div class="col-auto">
                        <button class="btn btn-danger shadow-sm fw-bold" onclick="AppMuseo.solicitarCargaExcel(4, 'solicitudes', 'ficha')">
                            🏛️ Solicitud (ID: 4 - Ficha)
                        </button>
                    </div>
                </div>
                ...
                <div class="inventory-section p-4 rounded mb-5">
                    ...
                    <div class="row gap-3 justify-content-start" id="inventario-items-container">
                        <p class="text-muted fst-italic" id="inv-vacio-msg">El inventario está vacío. Abre un acervo y agrégalo para guardarlo en la memoria dinámica.</p>
                    </div>
                </div>
            </div>
    ...
    vincularEventosGlobales() {
        // Vinculación de limpieza del tablero al destruir instancias
    ...
    /**
     * CONTROLADOR ASÍNCRONO DE CARGA (FETCH BINARIO)
     */
    solicitarCargaExcel(id, origen, tipoVisor) {
        const key = `${origen}_${id}`;
        
        // Si el objeto ya fue creado antes, su ciclo de vida persiste en memoria y solo lo reabrimos
        ...
        fetch(rutaArchivo)
            ...
            .then(buffer => {
                ...
                if (filaEncontrada) {
                    // Instanciamos la clase hija encargada del visor pasándole la data real
...
class TarjetaAcervo {
    constructor(id, origen, datosFila, tipoVisor, appMaestra) {
        ...
        this.app = appMaestra; // Referencia de comunicación entre objetos (Inyección de dependencias)
        this.guardado = false; 
    ...
    inyectarAInventario() {
        ...
        // Accedemos a la memoria de la instancia de la aplicación maestra de forma directa
